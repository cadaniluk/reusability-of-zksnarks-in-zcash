...
\title{Performance Optimization of zk-SNARKs For Special Cases of The Zcash Protocol}
...
A commonly used solution by cryptocurrencies to solve the double-spending problem is a public ledger called the blockchain \cite{nakamoto:bitcoin}.
Assuming that the validity of transactions is ensured by other means, the public nature of this data structure enables any node in the cryptocurrency's network to verify that a certain amount of money has already been spent and prevent it from being spent again.
However, without appropriate measures a public ledger can compromise privacy and reveal information such as the identities of transaction participants.
...
A user may employ an arbitrary number of pseudonyms to hide their true identity and use methods such as CoinJoin \cite{anon:coinjoin}, but even these fail to \cite{reid:bitcoin-anon}.
...
Zcash still uses a blockchain, but to make stronger anonymity guarantees than Bitcoin, Zcash offers tranparent transactions inherited from Bitcoin as well as shielded transactions.
...
The various properties of zk-SNARKs render them useful in Zcash: they are zero-knowledge, hence proving statements on auxiliary inputs without revealing them.
Their succinctness makes for proofs of constant size.
...
Finally, they are arguments of knowledge, meaning that a malicious sender cannot fake proofs to mislead nodes in the network in a practical setting. % TODO: includes "argument" part, perhaps add to explain "of knowledge" part...

Beyond its theoretical definition, zk-SNARKs steadily evolve to make them even more usable. In Zcash alone, three different zk-SNARKs \cite{bensasson:zksnark} \cite{groth:zksnark} \cite{hopwood:zcash} have been used throughout the protocol's history.
