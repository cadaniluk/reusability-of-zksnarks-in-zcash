...
Similarly, network updates gradually add complexity, but the most recent one has already been active for almost one year \cite{zcash:nu5}, which makes it worth investigating.

The Zcash source code is online \cite{zcash:github} and is consulted when testing on a Zcash node or on doubts about Zcash's implementation.
Most of the time is spent on reading relevant papers.
